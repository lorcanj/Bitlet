...
Usage: python3 tester.py OPTIONS TEST.in ...
   ...
   OPTIONS may include
       ...
       --progdir=DIR  Directory or JAR files containing bitlet application
...
def doExecute(cmnd, dir, timeout, line_num):
    ...
    try:
        ...
        if DEBUG:
            print("[line {}]: bitlet {}".format(line_num, cmnd))
            input_prompt = ">>> "
    ...
    try:
        ...
        if prog_dir is None:
            ...
            while k > 0 and access(prog_dir, W_OK):
                k -= 1
                if exists(join(prog_dir, 'bitlet', 'Main.class')):
                    break
                prog_dir = dirname(prog_dir)
            else:
                print("Could not find bitlet.Main.", file=sys.stderr)
                sys.exit(1)
    except GetoptError:
        Usage()
    if not files:
