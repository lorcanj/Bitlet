...
The instructions each have one of the following forms:
   ...
   T N    Set the timeout for bitlet commands in the rest of this test to N
          seconds.
   + NAME F
   ...
   ...
   <<<
          Run bitlet.Main with COMMAND ARGUMENTS as its parameters.  Compare
          ...
          or JAR file containing the bitlet.Main program is assumed to be
    ...
    try:
        ...
        if DEBUG:
            ...
            if next_cmd == "n":
                ...
                print(f"Ready to debug the command `bitlet {cmnd}`")
        ...
        return ("java bitlet.Main exited with code {}".format(excp.args[0]),
                excp.output)
...
if __name__ == "__main__":
    ...
    superverbose = False
    src_dir = 'src'
    gitlet_dir = join(dirname(abspath(getcwd())), "bitlet")
    output_tolerance = 0
    ...
    if not isdir(lib_dir):
        ...
        print(DIRECTORY_LAYOUT_ERROR.format("bitlet"))
        sys.exit(1)
    ...
    cleanTempDir(join(abspath(getcwd()), "bitlet"))
